...
public class ApplicationRuunerPrinter implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
            ...
            .forEach(ps -> {
                if (ps instanceof MapPropertySource mapPropertySource) {
                    mapPropertySource.getSource().keySet()
                        .stream()
                        .filter(k -> (k.startsWith("cosmos.queries")))
                        .forEach(k -> {
                                log.debug("cosmos.queries key={}  value={}", k, mapPropertySource.getSource().get(k));
                            }
                        );
                } else {
        ...
        log.debug("Application is ready for analytical report {}", env.getProperty("cosmos.queries.premiumByPlanTypeTierAnalysis"));
    }
}
